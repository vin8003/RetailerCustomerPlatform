    ...
    try:
        ...
        with transaction.atomic():
            ...
            if config.is_referral_enabled:
                referral = CustomerReferral.objects.select_for_update().filter(retailer=order.retailer, referee=order.customer, is_rewarded=False).first()
                if referral and order.total_amount >= config.min_referral_order_amount:
                    referrer_loyalty = _get_loyalty_for_update(referral.referrer, order.retailer)
                    referrer_loyalty.points += config.referral_reward_points
                    referrer_loyalty.save(update_fields=['points', 'updated_at'])
                    LoyaltyTransaction.objects.create(customer=referral.referrer, retailer=order.retailer, amount=config.referral_reward_points, transaction_type='earn', description=f"Referral reward (for referee {order.customer.username})", expiry_date=expiry_date)

                    referee_loyalty = _get_loyalty_for_update(order.customer, order.retailer)
                    referee_loyalty.points += config.referee_reward_points
                    referee_loyalty.save(update_fields=['points', 'updated_at'])
                    LoyaltyTransaction.objects.create(customer=order.customer, retailer=order.retailer, amount=config.referee_reward_points, transaction_type='earn', description=f"Referral reward (referred by {referral.referrer.username})", expiry_date=expiry_date)

                    referral.is_rewarded = True
                    referral.save(update_fields=['is_rewarded'])
        return True
