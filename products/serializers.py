...
def product_has_track_serial_field(model=None):
    """True when Product (or a dummy stand-in) declares track_serial."""
    target = Product if model is None else model
    meta = getattr(target, '_meta', None)
    if meta is None:
        return False
    try:
        meta.get_field('track_serial')
...
class TrackSerialReadMixin:
    """Optional track_serial on product reads. Not declared on Meta.fields."""

    def to_representation(self, instance):
        data = super().to_representation(instance)
        return attach_track_serial(data, instance)
...
class ProductListSerializer(TrackSerialReadMixin, PurchaseMarginReadMixin, SaleableQuantityReadMixin, FractionalChildrenReadMixin, GroupVariantsReadMixin, ChannelPriceRepresentationMixin, serializers.ModelSerializer):
    """
...
class ProductSearchSerializer(TrackSerialReadMixin, PurchaseMarginReadMixin, SaleableQuantityReadMixin, FractionalChildrenReadMixin, GroupVariantsReadMixin, ChannelPriceRepresentationMixin, serializers.ModelSerializer):
    """
...
class ProductDetailSerializer(TrackSerialReadMixin, PurchaseMarginReadMixin, SaleableQuantityReadMixin, FractionalChildrenReadMixin, GroupVariantsReadMixin, ChannelPriceRepresentationMixin, serializers.ModelSerializer):
    """
