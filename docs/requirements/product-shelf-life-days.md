...
POS `?no_page=true` reuses `ProductListSerializer`, so it inherits the same optional key. `ProductSearchSerializer` Meta is unchanged.
...
| Optional `shelf_life_days` on list / detail (and POS no_page via list serializer) | EXTEND |
...
| GET | `/api/products/?no_page=true` | Authenticated retailer | same (list serializer) |
...
Search Meta, GST/HSN rebuild, batch expiry policy (OE-136), inventory.adjust, pack write, timeline/OFD/khata/UPI/slots, FE, merge, Jira Done, live `*.ordereasy.win`.
