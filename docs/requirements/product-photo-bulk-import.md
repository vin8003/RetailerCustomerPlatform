...
Catalog match loads **only products that can match the import keys**, not the whole shop. Row cap and match result cap are both **200** (`MAX_ROWS` / `MAX_MATCH_PRODUCTS`). The match path prefetches `batches` (needed for batch barcodes) and does **not** prefetch `additional_images` (`is_primary` is cleared with an UPDATE on attach).
