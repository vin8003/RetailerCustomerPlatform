...
- **Ticket:** [OE-118](https://vin8003.atlassian.net/browse/OE-118) · backlog `F-0046` (preview) · [OE-169](https://vin8003.atlassian.net/browse/OE-169) · [OE-284](https://vin8003.atlassian.net/browse/OE-284) · backlog `F-0071` (catalog / POS field) · [snapshots](../tickets/OE-118.md)
...
| POS `GET /api/products/?no_page=true` `margin_percent` | EXTEND — same math on purchase-role POS catalog (OE-284) |
...
| GET | Retailer product list / detail / search | Purchase-role | Same `margin_percent` (2 d.p.). Cashiers and public **omit** the field. |
| GET | `/api/products/?no_page=true` | Purchase-role | Same `margin_percent` on the hand-built POS catalog. Cashiers **omit** the field. |
...
Cross-tenant SKU → **404** on detail / absent from list. Dedicated preview: cashier / customer → **403**. Catalog list/detail/search and POS `no_page`: non-purchase **omits** `margin_percent` (catalog stays readable). Unauthenticated → **401**.

At most one `PurchaseItem` query on the preview (skipped when draft cost is set). Catalog lists and POS `no_page` batch last-PI costs in one query. No N+1.
