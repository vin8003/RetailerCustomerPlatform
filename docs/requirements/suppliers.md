...
`supplier_name`). Optional `supplier_code` is list-only when the invoice or
joined supplier has that attribute — see [purchase-invoice-list-supplier-code.md](purchase-invoice-list-supplier-code.md).
`?search=` is wired on the viewset (`SearchFilter`) and uses `invoice_number` and
