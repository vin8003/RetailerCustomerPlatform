...
| `purchasing.terms` | Set or change `Supplier.payment_terms` (OE-100). Echoing the current value (or creating a supplier without terms) does not require the perm. Also the purchase-role gate for last supplier-cost reads (OE-112) and SKU margin% preview (OE-118). Admin bootstrap includes it; cashier does not. |
