...
Retailer product **reads** may include top-level `reorder_level` when `Product` actually has that field **and** the caller is an authenticated retailer. This repo's `Product` model does not declare it yet, so current payloads **omit** the key. Do not add the name to serializer `Meta.fields` — a missing model field would raise at import/bind time.

When both conditions hold, null stays `null` (no invented `0`). `0` is a real threshold and is echoed. Quantity-style JSON: integral Decimals become ints.

Customer / anonymous / missing-request callers never receive the key (same fail-closed idea as store-only price fields). Shared list/detail/search serializers are used on public catalog; the mixin must not leak shop ops there the day the column lands.
...
| `reorder_level` on list / detail / search via `to_representation` | EXTEND (only if field exists **and** caller is retailer) |
| Public / customer / anonymous catalog using the same serializers | EXISTING — key omitted |
