...
- Sale deducts (POS and customer `place_order`) cannot take on-hand below zero unless the caller already passes `allow_negative=True` on `Product.reduce_quantity`. POS accepts that flag as JSON `true` only. See [block-negative-stock.md](../requirements/block-negative-stock.md).
